<template>
  <div class="followers-list">
    ...
    <div class="loading-container" v-if="loading && followers.length === 0">
      <el-skeleton :rows="5" animated />
    </div>

    <div class="list-content" v-if="followers.length > 0">
    ...
    <div class="no-more" v-if="noMore && followers.length > 0">
      <span>没有更多了</span>
    </div>
...
import { getFollowers, followUser, unfollowUser, checkFollowing } from '@/api/user';
...
const getFollowersList = async (isLoadMore = false) => {
  ...
  try {
    const response = await getFollowers(props.userId, currentPage.value);
    const newFollowers = response.data.followers || [];
    
    // 转换数据格式，只保留用户信息
    const formattedFollowers = newFollowers.map((item: any) => ({
      id: item.followerId,
    ...
    if (isLoadMore) {
      followers.value = [...followers.value, ...formattedFollowers];
    } else {
      followers.value = formattedFollowers;
    }
    
    // 检查关注状态
    for (const user of formattedFollowers) {
